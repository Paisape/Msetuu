import { NextResponse } from 'next/server'

import prisma from '@/libs/prisma'
import { requireUser, requireAdmin, handleApiError } from '@/libs/api-auth'
import { logOrderTrail } from '@/libs/orderTrail'
import { cancelInvoiceAndRefund } from '@/libs/invoice'

type Params = { params: Promise<{ id: string }> }

const VALID_STATUSES = new Set(['PENDING', 'CONFIRMED', 'COMPLETED', 'CANCELLED'])
const VALID_PAYMENT_STATUSES = new Set(['PENDING', 'PAID', 'FAILED'])

// GET /api/jyotish/[id] — booking owner or an admin can view a single booking
export async function GET(_req: Request, { params }: Params) {
  try {
    const user = await requireUser()
    const { id } = await params

    const booking = await prisma.consultationBooking.findUnique({
      where: { id },
      include: { astrologer: true, user: { select: { name: true, email: true } } }
    })

    if (!booking) return NextResponse.json({ error: 'Booking not found.' }, { status: 404 })

    if (booking.userId !== user.id && user.role !== 'ADMIN') {
      return NextResponse.json({ error: 'You do not have access to this booking.' }, { status: 403 })
    }

    return NextResponse.json(booking)
  } catch (err) {
    return handleApiError(err)
  }
}

// PATCH /api/jyotish/[id] — admin assigns an astrologer and/or confirms the slot
export async function PATCH(req: Request, { params }: Params) {
  try {
    const admin = await requireAdmin()

    const { id } = await params
    const body = await req.json()
    const { status, paymentStatus, astrologerId, slotTime } = body

    const data: Record<string, unknown> = {}

    if (status !== undefined) {
      if (!VALID_STATUSES.has(status)) {
        return NextResponse.json({ error: `status must be one of ${[...VALID_STATUSES].join(', ')}` }, { status: 400 })
      }

      data.status = status
    }

    if (paymentStatus !== undefined) {
      if (!VALID_PAYMENT_STATUSES.has(paymentStatus)) {
        return NextResponse.json(
          { error: `paymentStatus must be one of ${[...VALID_PAYMENT_STATUSES].join(', ')}` },
          { status: 400 }
        )
      }

      data.paymentStatus = paymentStatus
    }

    if (astrologerId !== undefined) {
      if (astrologerId !== null) {
        const astro = await prisma.astrologer.findUnique({ where: { id: astrologerId } })

        if (!astro) return NextResponse.json({ error: 'Astrologer not found.' }, { status: 404 })
      }

      data.astrologerId = astrologerId
    }

    if (slotTime !== undefined) {
      const parsedSlot = new Date(slotTime)

      if (Number.isNaN(parsedSlot.getTime())) {
        return NextResponse.json({ error: 'slotTime must be a valid date/time.' }, { status: 400 })
      }

      data.slotTime = parsedSlot
    }

    const booking = await prisma.consultationBooking.update({
      where: { id },
      data,
      include: { astrologer: true, user: { select: { name: true, email: true } } }
    })

    if (status !== undefined) {
      await logOrderTrail({ orderType: 'JYOTISH', orderId: id, status, actorId: admin.id, actorRole: 'ADMIN', req })

      if (status === 'CANCELLED') {
        await cancelInvoiceAndRefund('JYOTISH', id)
      }
    }

    return NextResponse.json(booking)
  } catch (err) {
    return handleApiError(err)
  }
}
