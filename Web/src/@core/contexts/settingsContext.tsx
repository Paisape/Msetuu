...
export const SettingsProvider = (props: Props) => {
  ...
  // Theme is light-only (dark/system mode switcher removed sitewide) — 'light' is forced here
  // unconditionally so a cookie saved before this change (mode: 'dark'/'system') can never bring
  // dark mode back for a returning visitor.
  ...
  const [settingsCookie, updateSettingsCookie] = useObjectCookie<Settings>(
    ...
    JSON.stringify(props.settingsCookie) !== '{}' ? { ...props.settingsCookie, mode: 'light' } : updatedInitialSettings
  )

  // State
  const [_settingsState, _updateSettingsState] = useState<Settings>(
    JSON.stringify(settingsCookie) !== '{}' ? { ...settingsCookie, mode: 'light' } : updatedInitialSettings
  )
